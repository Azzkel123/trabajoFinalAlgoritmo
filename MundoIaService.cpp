...
void MundoIAService::generarRecursos() {
	char rutaMoneda[] = "iaRecurso.png";

	// Leer posiciones desde archivo monedas.txt
	vector<vector<int>> posiciones = gestorArchivos->leerLineasInt("monedas.txt", '|');

	// Crear monedas con las posiciones leidas
	for (int i = 0; i < posiciones.size(); i++) {
		if (posiciones[i].size() >= 2) {
			int x = posiciones[i][0];
			int y = posiciones[i][1];
			IaRecurso* recurso = new IaRecurso(x, y);
			recurso->cargarImagen(rutaMoneda, 4, 4);
			recursos.push_back(recurso);
		}
	}
}
...
void MundoIAService::generarObstaculos() {
	...
	for (int i = 0; i < posiciones.size(); i++) {
		if (posiciones[i].size() >= 2) {
			int x = posiciones[i][0];
			int y = posiciones[i][1];
			Mando* obstaculo = new Mando(x, y, 1);
			obstaculo->cargarImagen(rutaMoneda, 4, 4);
			obstaculos.push_back(obstaculo);
		}
	}
}
